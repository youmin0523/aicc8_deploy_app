...
import NavbarV2 from '../v2/Navbar_v2';
import ModalV2 from '../v2/Modal_v2';
...
// //* [Mentor's Encyclopedia: CalendarViewV2 (Restore)]
// //* 1. UI 복구: 'v2-calendar' 클래스와 'task-hover-popup' 구성을 통해 정교한 그리드 오버레이 UI를 복구했습니다.
// //* 2. 기능 복구: 날짜별 일정 필터링, 공휴일 표시, 호버 팝업 및 상세 사이드바 연동 로직을 전수 복구했습니다.
// //* 3. 보정: 모달 오픈 시 팝업 가림 처리를 위해 부모 컨테이너에 'modal-is-open' 클래스를 동적으로 부여합니다.
...
const CalendarViewV2 = () => {
  ...
  useEffect(() => {
    if (auth?.sub) dispatch(fetchGetItemV2(auth.sub));
  }, [auth, dispatch]);
  ...
  const getTileContent = ({ date, view }) => {
    ...
    // //* [UI 정비] 5열/6열(금,토)일 경우 팝업이 왼쪽으로 뜨게 하여 화면 밖으로 나가는 현상 방지
        ...
        {dayTasks.length > 0 && (
          <>
            <div className="calendar-task-list pointer-events-auto">
              ...
              {dayTasks.length > 2 && (
                <div className="text-[8px] text-gray-500 pl-1 font-bold">
                  +{dayTasks.length - 2} items
                </div>
              )}
            </div>

            {/* //* [UI 복구] CSS와 연동되는 호버 팝업 레이어 구조 */}
            <div
  ...
  return (
    <div
      className={`page_section bg-[#0f0f0f] min-h-screen flex ${isOpen ? 'modal-is-open' : ''}`}
    >
      <NavbarV2 />
      <main className="flex-1 p-8 flex flex-col lg:flex-row gap-8 overflow-y-auto custom-scrollbar">
        <div className="flex-1 min-w-0">
          ...
          <div className="calendar-container bg-[#121212] p-8 rounded-[2.5rem] border border-gray-800 shadow-[0_30px_60px_rgba(0,0,0,0.5)] overflow-visible">
            <Calendar
        ...
        <aside className="w-full lg:w-[350px] shrink-0">
          <div className="bg-[#121212] h-full rounded-[2.5rem] p-8 border border-gray-800 shadow-2xl flex flex-col">
